...
void Adafruit_SH1106<WIDTH, HEIGHT>::sh1106_command1(uint8_t c) {
  uint8_t cmd = 0x00; // Co = 0, D/C = 0
  TWI_Start_Transceiver_With_Data(cmd, &c, 1);
}

// Issue list of commands to , same rules as above re: transactions.
// This is a private function, not exposed.
template <uint8_t WIDTH, uint8_t HEIGHT>
void Adafruit_SH1106<WIDTH, HEIGHT>::sh1106_commandList(const uint8_t *c, uint8_t n) {
  uint8_t cmd = 0x00; // Co = 0, D/C = 0
  uint8_t tmp[n];
...
*/
template <uint8_t WIDTH, uint8_t HEIGHT>
bool Adafruit_SH1106<WIDTH, HEIGHT>::begin(uint8_t vcs, uint8_t addr) {
 ...
 sh1106_commandList(init1, sizeof(init1));
  sh1106_command1(HEIGHT - 1);
  ...
  sh1106_commandList(init2, sizeof(init2));

  sh1106_command1((vccstate == SH1106_EXTERNALVCC) ? 0x10 : 0x14);
  ...
  static const uint8_t PROGMEM init3[] = {
    ...
    SH1106_SEGREMAP | 0x1,
    SH1106_COMSCANDEC };
  sh1106_commandList(init3, sizeof(init3));
  ...
  if((WIDTH == 128) && (HEIGHT == 32)) {
    static const uint8_t PROGMEM init4a[] = {
      ...
      0x8F };
    sh1106_commandList(init4a, sizeof(init4a));
  } else if((WIDTH == 128) && (HEIGHT == 64)) {
    static const uint8_t PROGMEM init4b[] = {
      SH1106_SETCOMPINS,                 // 0xDA
      0x12,
      SH1106_SETCONTRAST };              // 0x81
    sh1106_commandList(init4b, sizeof(init4b));
    sh1106_command1((vccstate == SH1106_EXTERNALVCC) ? 0x9F : 0xCF);
  } else if((WIDTH == 96) && (HEIGHT == 16)) {
    static const uint8_t PROGMEM init4c[] = {
      SH1106_SETCOMPINS,                 // 0xDA
      0x2,    // ada x12
      SH1106_SETCONTRAST };              // 0x81
    sh1106_commandList(init4c, sizeof(init4c));
    sh1106_command1((vccstate == SH1106_EXTERNALVCC) ? 0x10 : 0xAF);
  } else {
    // Other screen varieties -- TBD
  }

  sh1106_command1(SH1106_SETPRECHARGE); // 0xd9
  sh1106_command1((vccstate == SH1106_EXTERNALVCC) ? 0x22 : 0xF1);
  static const uint8_t PROGMEM init5[] = {
    SH1106_SETVCOMDETECT,               // 0xDB
    0x40,
    SH1106_DISPLAYALLON_RESUME,         // 0xA4
    SH1106_NORMALDISPLAY,               // 0xA6
    SH1106_DEACTIVATE_SCROLL,
    SH1106_DISPLAYON };                 // Main screen turn on
  sh1106_commandList(init5, sizeof(init5));
...
*/
template <uint8_t WIDTH, uint8_t HEIGHT>
void Adafruit_SH1106<WIDTH, HEIGHT>::drawPixel(int16_t x, int16_t y, uint16_t color) {
  if((x >= 0) && (x < WIDTH) && (y >= 0) && (y < HEIGHT)) {
    // Pixel is in-bounds. Rotate coordinates if needed.
    switch(color) {
     case SH1106_WHITE:   buffer[x + (y/8)*WIDTH] |=  (1 << (y&7)); break;
     case SH1106_BLACK:   buffer[x + (y/8)*WIDTH] &= ~(1 << (y&7)); break;
     case SH1106_INVERSE: buffer[x + (y/8)*WIDTH] ^=  (1 << (y&7)); break;
    }
  }
}
...
*/
template <uint8_t WIDTH, uint8_t HEIGHT>
void Adafruit_SH1106<WIDTH, HEIGHT>::clearDisplay(void) {
...
*/
template <uint8_t WIDTH, uint8_t HEIGHT>
void Adafruit_SH1106<WIDTH, HEIGHT>::drawFastVLine(
  int16_t x, int16_t y, int16_t h, uint16_t color) {
  drawFastVLineInternal(x, y, h, color);
}

template <uint8_t WIDTH, uint8_t HEIGHT>
void Adafruit_SH1106<WIDTH, HEIGHT>::drawFastVLineInternal(
  ...
  if((x >= 0) && (x < WIDTH)) { // X coord in bounds?
    ...
    if(__h > 0) { // Proceed only if height is now positive
      ...
      if(mod) {
        ...
        switch(color) {
         case SH1106_WHITE:   *pBuf |=  mask; break;
         case SH1106_BLACK:   *pBuf &= ~mask; break;
         case SH1106_INVERSE: *pBuf ^=  mask; break;
        }
        pBuf += WIDTH;
      }
...
template <uint8_t WIDTH, uint8_t HEIGHT>
void Adafruit_SH1106<WIDTH, HEIGHT>::clearRect(uint8_t x, uint8_t y, uint8_t w , uint8_t h) {
...
template <uint8_t WIDTH, uint8_t HEIGHT>
void Adafruit_SH1106<WIDTH, HEIGHT>::drawBitmap(int16_t x, int16_t y,
...
*/
template <uint8_t WIDTH, uint8_t HEIGHT>
bool Adafruit_SH1106<WIDTH, HEIGHT>::getPixel(int16_t x, int16_t y) {
  if((x >= 0) && (x < WIDTH) && (y >= 0) && (y < HEIGHT)) {
    // Pixel is in-bounds. Rotate coordinates if needed.
...
*/
template <uint8_t WIDTH, uint8_t HEIGHT>
uint8_t *Adafruit_SH1106<WIDTH, HEIGHT>::getBuffer(void) {
  return buffer;
}
...
*/
template <uint8_t WIDTH, uint8_t HEIGHT>
void Adafruit_SH1106<WIDTH, HEIGHT>::display(void) {
  ...
  sh1106_commandList(dlist1, sizeof(dlist1));
  sh1106_command1(WIDTH - 1); // Column end address
...
*/
template <uint8_t WIDTH, uint8_t HEIGHT>
void Adafruit_SH1106<WIDTH, HEIGHT>::invertDisplay(bool i) {
